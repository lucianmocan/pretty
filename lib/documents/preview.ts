...
const memoryPreviewCache = new Map<string, StoredPagePreview>()

function validPreviewSnapshot(
  snapshot: StoredPagePreview | undefined,
  variant: string
): snapshot is StoredPagePreview {
  return Boolean(
    snapshot &&
...
/** Synchronous fast path for consumers that can reuse a snapshot already
 * read or generated by a thumbnail during this tab's lifetime. */
export function peekPagePreview(pageId: string, variant: string): PagePreviewSnapshot | null {
  const snapshot = memoryPreviewCache.get(pageId)
  return validPreviewSnapshot(snapshot, variant) ? snapshot : null
}
...
export async function readPagePreview(pageId: string, variant: string): Promise<PagePreviewSnapshot | null> {
  const inMemory = peekPagePreview(pageId, variant)
  ...
  try {
    ...
    if (!validPreviewSnapshot(snapshot, variant)) return null
    memoryPreviewCache.set(pageId, snapshot)
    return snapshot
  } catch {
...
export async function savePagePreview(
  ...
  const storedSnapshot: StoredPagePreview = {
    ...snapshot,
    rendererVersion: PREVIEW_RENDERER_VERSION,
    savedAt: Date.now(),
  }
  memoryPreviewCache.set(snapshot.pageId, storedSnapshot)
  try {
    const database = await openPreviewDatabase()
    if (!database) return
    const transaction = database.transaction(PREVIEW_STORE, 'readwrite')
    transaction.objectStore(PREVIEW_STORE).put(storedSnapshot)
...
export async function clearPagePreview(pageId: string): Promise<void> {
  memoryPreviewCache.delete(pageId)
  try {
    const database = await openPreviewDatabase()
    if (!database) return
    const transaction = database.transaction(PREVIEW_STORE, 'readwrite')
